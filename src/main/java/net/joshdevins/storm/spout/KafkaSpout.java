...
/**
 * This spout can be used to consume messages from a cluster of Kafka brokers. The consumption and further processing in the Storm topology is currently unreliable! When you create this spout in
 * Storm, it is recommended that the parallelism hint you set be less than or equal to the number of partitions available for the topic you are consuming from. If you specify a parallelism hint that
 * is great than the number of partitions, some spouts/consumers will sit idle and not do anything. This design is inherent to the way Kafka works.
 * 
 * @author Josh Devins
 */
public class KafkaSpout extends BasicSchemeSpout {
	 ...
	 * @param scheme
	 *            A {@link Scheme} that will be responsible for deserializing messages coming from Kafka. Whatever objects this produces needs to be natively understood by Storm or needs to be <a
	 *            href="https://github.com/nathanmarz/storm/wiki/Serialization">registered with Storm</a> as a serializable type. Your best bet however is to stick to "native" types and use the
	 *            internal tuple structure.
	 */
	...
	/**
	 * Consume one message from the Kafka segment on the broker. As per the documentation on {@link ISpout}, this method <strong>must</strong> be non-blocking if reliability measures are enabled/used,
	 * otherwise ack and fail messages will be blocked as well. This current implementation is not reliable so we use a blocking consumer.
	 */
	@Override
	public void nextTuple() {
		...
		byte[] byteMessage = consumerIterator.next().message();
	...
	public void activate() {
		 ...
		 * @desc
		 * 
		 * @created 2014-8-8 下午5:06:36 by zhanghl
		 * @version
		 */
	}
